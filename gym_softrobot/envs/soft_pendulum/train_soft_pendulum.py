...
def get_next_ppo_model_name(save_dir, base_name):
    ...
    existing_models = [f for f in os.listdir(save_dir) if re.match(rf"{base_name}_(\d+).zip", f)]
...
model = PPO("MlpPolicy", env, verbose=1, tensorboard_log="./tensorboard_log/",ent_coef=0.01, vf_coef=1e-8,gamma=0.90)
